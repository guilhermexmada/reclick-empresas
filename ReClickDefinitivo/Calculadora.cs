...
namespace ReClickDefinitivo
{
    public partial class Calculadora : Form
    {
        ...
        private void MontaGrafico()
        {
            ...
            //ds = gas.Consultar_Gastos(int.Parse(cmb_registros.SelectedValue.ToString()));

            //obtendo valor do id e passando para consulta
            int idSelecionado = Convert.ToInt32(cmb_registros.SelectedValue);
            ds = gas.Consultar_Gastos(idSelecionado);
            //MessageBox.Show("- " + cmb_registros.SelectedValue);
            ...
            if (ds.Rows.Count > 0)
            {
                ...
                cht_Gastos.Palette = ChartColorPalette.SeaGreen;
        ...
        public void Historico_Gastos()
        {
            ...
            string query = "SELECT id, data_hora FROM gastos WHERE id_usuario_empresa = '"+ emp.Id + "'; ";
            if (conn.abrirConexao() == true)
            {
               ...
               // while (reader.Read())
                //{ 
                    // Adicionar um item com a data/hora exibida e o ID como valor real
                 //   cmb_registros.Items.Add(new { Display = reader["data_hora"], Value = reader["id"] });
                    
                //}
                // Definir o DisplayMember e ValueMember do ComboBox
                cmb_registros.DisplayMember = "data_hora";
                cmb_registros.ValueMember = "id";
                cmb_registros.DataSource = dt;

                /*
                while (reader.Read())
                {

                    cmb_registros.Items.Add(reader["data_hora"].ToString());
                }
                */
        ...
        private void cht_Gastos_Click(object sender, EventArgs e)
        {
        ...
        private void txt_rejeitos_organicos_TextChanged(object sender, EventArgs e)
        {
